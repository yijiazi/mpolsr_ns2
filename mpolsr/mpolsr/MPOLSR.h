...
///
#define MPOLSR_REFRESH_INTERVAL	3
...
#define n_ 4        //the packets needed to do the coding
#define N_ 4		//the number of the projection
#define M_ 2		//the packet needed to do the decoding
...
class MojetteTimer : public TimerHandler {

public :

    MojetteTimer(int uid = 0);
...
public :
    ...
    static std::map<int, Packet*> sOriginals;  //the original packets for reconstruction, 1 to n

	static std::map<int, std::vector<Packet*> > sOriginals_N; //the original packets for reconstruction, N to n 

    static std::map<int, MojetteTimer> sTimers;

		std::map<int, std::vector< Packet*> > mSend;       //the send buffer, the key is the  destination
...
//the mojette transform
    std::vector<Packet*> burst(Packet* pkt, int n, int m, 
            std::vector<std::vector<nsaddr_t> >* addrs);

	std::vector<Packet*> burst(Packet* pkt, int N, int n, int m, 
		std::vector<std::vector<nsaddr_t> >* addrs);

//the reverse transform
	

    Packet* rebuild(Packet* pkt);

	std::vector<Packet*> rebuild(Packet* pkt, int t);

private :

    std::map<int, int > mReceived;       //the receive counter, the key is the packet ID
...
/// internal state.
///
class MPOLSR : public Tap, public Agent {
	// Makes some friends.
	...
	friend class MojetteTimer;
	//friend class Mojette;
